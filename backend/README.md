...
`recommend` принимает `dataset_hash`, `require_stress` (по умолчанию true), единственный
`method_id: "hybrid_maximin_v1"`, `cash_loss_limit_mrub` (null = автоматический Δ),
`quality_epsilon: 0` и дополнительные ограничения. Прежние методы и веса отклоняются с 422.
Результат — вычисленный победитель Q → S, шесть оценок с фиксированными шкалами,
фактический Δ, денежный порог, все точки смены победителя и независимые сценарии чувствительности.
Канонические метрики не округляются; ранжирование удаляет машинный шум до 8 знаков
и далее сравнивает рациональные значения точно. Метод и API имеют версию 2.0.0.
Подробный контракт: `core/dto/portfolio.py`; чистое правило: `portfolio_engine/hybrid.py`.
