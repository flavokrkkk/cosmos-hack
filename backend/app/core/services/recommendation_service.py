...
# по `vpub`. Максимум `vpub` на фронте делит РОВНО ОДНА конфигурация, поэтому
# остальные восемь приоритетов не срабатывали никогда, и метод вырождался
# в «максимизируй общественную ценность». Инструмент выдавал
