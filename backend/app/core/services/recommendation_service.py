...
from app.core.services.portfolio_ranking_service import METHODS, PARETO_MAXIMIZE, analyze
...
def _recommend_ranked(serialized_request: str) -> RecommendationResult:
    ...
    front = space.pareto_front(candidates, maximize=PARETO_MAXIMIZE)
    method = next(method for method in METHODS if method.id == request.method_id)
    result = RecommendationResult(
    ...
    primary = next(outcome for outcome in analysis.methods if outcome.method_id == request.method_id)
    result.recommended = variant(primary.selection_id, method.title, method.description)
    used = {primary.selection_id}
    for outcome in analysis.methods:
        if outcome.selection_id not in used:
            other = next(item for item in METHODS if item.id == outcome.method_id)
            result.alternatives.append(variant(outcome.selection_id, other.title, other.description))
            used.add(outcome.selection_id)
    for column, ascending, title, reason in REFERENCE_POINTS[:3]:
        row = front.sort_values([column, "stable_id"], ascending=[ascending, True]).iloc[0]
        if row.stable_id not in used and len(result.alternatives) < 4:
    ...
    def recommend(self, request: RecommendRequest) -> RecommendationResult:
        ...
        with _search_lock:
            result = (_recommend(request.dataset_hash, request.require_stress, lot_ids)
                      if request.method_id == "pareto_lexicographic_v1" else _recommend_ranked(request.model_dump_json()))
