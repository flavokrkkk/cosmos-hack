...
def metrics_row(selection: Selection) -> Dict[str, object]:
    ...
    row: Dict[str, object] = {
        ...
        "c0": round(float(metrics["c0_mrub"]), 4),
        "opex": round(float(metrics["opex_mrub_per_year"]), 4),
        "vpub": round(float(metrics["vpub_mrub_per_year"]), 4),
        "cash": round(float(metrics["cash_mrub_per_year"]), 4),
        "kcash": round(float(metrics["kcash"]), 6),
        "t_rep": round(float(metrics["t_rep"]), 6),
        "readiness": round(float(metrics["readiness_1_5"]), 4),
        "resilience": round(float(metrics["resilience_1_5"]), 4),
        "scale": round(float(metrics["scale_1_5"]), 4),
