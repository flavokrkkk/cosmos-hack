...
| `features` | `recommend-portfolio` (автоподбор и подбор режимов в два шага, открытый вариант, `explanationFor`, тумблер STRESS, счётчики, `SearchSettings` — Δ, лимит C0, минимум VPUB, обязательное общественное ядро), `edit-portfolio` (ручной выбор), `compare-portfolios` (кандидаты + диалог с AI-анализом), `save-variant` (диалоги сохранения и списка), `export-calculation` |
| `widgets` | `ModeSwitch`, `PortfolioReview` (текущий портфель + проверка + ограничения + действия), `Alternatives`, `ExplanationBlock`, `DecisionAnalysis` (стадии правила выбора, шесть оценок, интервалы Δ, чувствительность), `LotDetailsHost`, `SolutionMaterials`, `PageFooter` |
