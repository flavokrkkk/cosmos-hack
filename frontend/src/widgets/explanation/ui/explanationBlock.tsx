...
export function ExplanationBlock({ result, isLoading, errorMessage, onRetry, className }: Props) {
  return (
    ...
    >
      ...
      <Card
        className={cn(
          'relative z-10 mb-4 flex w-full max-w-[300px] flex-col gap-4 bg-card px-6 pt-5 pb-6 backdrop-blur-[8px]',
      ...
      <Panel className="relative min-h-[190px] bg-panel px-7 pt-6 pb-7 lg:pt-[104px]" aria-busy={isLoading}>
        ...
        {isLoading && !result ? (
          ...
          <UnavailableExplanation disabled={result.unavailable_reason === 'disabled'} onRetry={onRetry} isLoading={isLoading} />
        ) : result ? (
          <ExplanationText result={result} isLoading={isLoading} />
        ) : (
          <div className="flex min-h-[92px] items-center justify-center rounded-card bg-sunken px-5 text-center backdrop-blur-[8px]">
...
function StatusLabel({ isLoading, result }: { isLoading?: boolean; result: RecommendationExplanation | null | undefined }) {
  ...
  if (result) return <Tag tone="muted" size="md">{result.unavailable_reason === 'disabled' ? 'AI отключён' : 'AI недоступен'}</Tag>
...
function UnavailableExplanation({
  ...
  return (
    <div className="flex min-h-[92px] flex-col items-start justify-center gap-3">
      <p className="text-[14px] text-muted">
        {disabled
          ? 'AI-объяснение отключено. Расчёт портфеля доступен.'
          : 'Не удалось получить AI-объяснение. Расчёт портфеля доступен.'}
      </p>
      {!disabled && onRetry ? (
