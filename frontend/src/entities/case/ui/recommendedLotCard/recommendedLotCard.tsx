...
export function RecommendedLotCard({
  ...
  return (
    <Card
      ...
      className={cn(
        'flex min-h-[184px] w-full flex-col p-4 transition-[rotate,box-shadow] duration-300 ease-(--ease-soft)',
    ...
    >
      ...
      <dl className="mt-auto flex gap-4 border-t border-line/70 pt-3">
        <div>
          <dt className="text-[10px] font-bold tracking-[0.02em] text-ink/45" title={`С учётом режима ${modeId}`}>C0</dt>
          <dd className="mt-1 text-[13px] font-semibold text-ink/50 tabular-nums">{formatMoney(detail.c0_mrub)}</dd>
        </div>
        <div>
          <dt className="text-[10px] font-bold tracking-[0.02em] text-ink/45" title={`С учётом режима ${modeId}`}>VPUB</dt>
      ...
      <dl className="mt-3 border-t border-line/70 pt-3">
        <dt className="text-[10px] font-bold tracking-[0.02em] text-ink/45">{modeLabel}</dt>
        <dd className="mt-1 text-[16px] font-semibold">{modeId}</dd>
      </dl>
    </Card>
  )
}
