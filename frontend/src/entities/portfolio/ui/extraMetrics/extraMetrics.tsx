 ...
 */
export function ExtraMetrics({ metrics, shown = [], rest = [], className }: Props) {
  const shownKeys = new Set(shown.map((tile) => tile.key))
  const tiles = [...rest, ...EXTRA_METRIC_TILES].filter((tile) => !shownKeys.has(tile.key))

  return (
    <Collapsible title={<span className="text-[15px] font-semibold">Подробнее о портфеле</span>} className={className}>
