...
export function PortfolioLotCard({ lot, mode, detail, onDetails, className }: Props) {
  ...
  const content = (
    <>
      ...
      <h3 className="mt-4 text-[16px] leading-tight font-bold" title={lot.title}>
        {lot.lot_id}
      </h3>
      ...
      <dl className="mt-7 flex flex-col gap-2">
        {rows.map((row) => (
          <div key={row.label}>
            <dt className="text-[10px] font-bold tracking-[0.02em] text-ink/45">{row.label}</dt>
            <dd className="mt-0.5 text-[14px] font-semibold text-ink/60 tabular-nums">
    ...
    return (
      <Card className={cn('p-0', className)}>
        <button
          ...
          className="block h-full w-full rounded-card p-3 text-left transition-shadow duration-300 ease-(--ease-soft) hover:shadow-card-hover"
  ...
  return <Card className={cn('p-3', className)}>{content}</Card>
}
