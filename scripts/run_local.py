...
def main() -> int:
    ...
    if args.ollama == "on":
        print("Модель загружается в volume. До завершения доступны объяснения по расчётным данным.")
        print("Прогресс: docker compose logs -f ollama-pull (с теми же --project-name/--env-file, если заданы).")
    elif args.ollama == "host":
        print("Используется установленная Ollama. При её недоступности объяснения строятся по расчётным данным.")
    else:
        print("Ollama отключена. Расчёты, сравнение, сохранение, экспорт и объяснения по данным доступны.")
    return 0
